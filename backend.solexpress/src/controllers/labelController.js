...
const { API_LABELS_CONFIG, LABEL_MESSAGES } = require("../config/api-labels");
...
async function handleApiResponse(response, line, userId) {
  ...
  if (!data) {
    return { success: false, error: LABEL_MESSAGES.DEFAULT_ERROR };
  }

  if (data.ResultCode === 0 || data.ResultDesc === LABEL_MESSAGES.SUCCESS_CN) {
    return handleSuccessResponse(data, line, userId);
  }

  if (
    data.ResultCode === 1001 ||
    data.ResultDesc === LABEL_MESSAGES.FAILURE_CN
  ) {
...
async function handleSuccessResponse(data, line, userId) {
  ...
  const item = result[0];
  const status =
    data.ResultDesc?.replaceAll(
      LABEL_MESSAGES.SUCCESS_CN,
...
function handleFailureResponse(data, line) {
  ...
  const item = result[0];
  const errorMessage =
    item?.Feedback?.replaceAll(
      LABEL_MESSAGES.DUPLICATE_ORDER,
      LABEL_MESSAGES.DUPLICATE_ORDER_VI,
    ) ?? LABEL_MESSAGES.DEFAULT_ERROR;
...
const importLabels = async (req, res) => {
  if (!req.file || req.file.mimetype !== "text/csv") {
    return ApiResponse.badRequest(
      res,
      "File không đúng. Vui lòng upload file CSV.",
    );
  }
